...
import {
  LayoutDashboard, Users, Church, UserCog, Settings, LogOut, ChevronLeft, ChevronRight,
...
interface Props {
  /** Em telemóvel o painel ocupa a gaveta lateral e não recolhe. */
  mobile?: boolean;
  onNavigate?: () => void;
}

const AppSidebar = ({ mobile = false, onNavigate }: Props) => {
  ...
  const recolhido = mobile ? false : collapsed;
  ...
  const links = [
    ...
    ...(isDistrital ? [{ to: "/auditoria", icon: History, label: "Auditoria" }] : []),
  ...
  return (
    <aside className={cn(
      ...
      mobile ? "w-full" : recolhido ? "w-[70px]" : "w-[250px]",
    )}>
      ...
      <div className="border-t border-sidebar-border p-2 space-y-1">
        {!mobile && (
          <button onClick={() => setCollapsed(!collapsed)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-sidebar-foreground/70 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground w-full transition-colors">
            {recolhido ? <ChevronRight size={20} /> : <ChevronLeft size={20} />}
