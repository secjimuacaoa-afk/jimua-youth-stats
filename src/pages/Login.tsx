...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      toast({ title: "Erro no login", description: "Email ou senha incorrectos.", variant: "destructive" });
      return;
    }
        ...
        <CardContent className="px-8 pb-8">
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="email" type="email" placeholder="seu.email@jimua.org" value={email} onChange={(e) => setEmail(e.target.value)} required />
            </div>
            ...
            <Button type="submit" className="w-full bg-primary hover:bg-navy-dark text-primary-foreground font-semibold" disabled={loading}>
              {loading ? (
